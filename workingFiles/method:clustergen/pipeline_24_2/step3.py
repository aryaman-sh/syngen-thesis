"""
STEP3: SAMPLING PRIORS FOR LABELS IN CLUSTERED
"""
...
# third-party imports
from lab2im.lab2im import utils
from lab2im.lab2im import edit_volumes


def sample_intensity_stats_from_image(image, segmentation, labels_list, classes_list=None, keep_strictly_positive=False):
...
def build_intensity_stats(list_image_dir,
    ...
    # save files
    np.save(os.path.join(result_dir, f'prior_means_{tn}_{kv}_r{run+1}.npy'), prior_means)
    np.save(os.path.join(result_dir, f'prior_stds_{tn}_{kv}_r{run+1}.npy'), prior_stds)

    return prior_means, prior_stds

if __name__ == "__main__":
    # load image and segmentation as numpy array
    
    """
    For all images, make a temporary directory for image and label
    remember ary
        image: Actual training image
        label: cluster
    """
        ...
        for tn in train_names:
            for kv in k_vals:
                ...
                with tempfile.TemporaryDirectory() as tempdir1, tempfile.TemporaryDirectory() as tempdir2:
                    shutil.copy(f'/scratch/itee/uqasha24/synthetic-generalisation/fatdata/images/sub-{tn}.FatImaging_W.nii.gz', tempdir1) # IMAGE
                    shutil.copy(f'/scratch/itee/uqasha24/thesis2024/samplingBased/pl1/step1data/{tn}_{kv}_r{run+1}.nii.gz', tempdir2) # LABEL
                    build_intensity_stats(f"{tempdir1}", f"{tempdir2}", "/scratch/itee/uqasha24/thesis2024/samplingBased/pl1/step3data", f"/scratch/itee/uqasha24/thesis2024/samplingBased/pl1/step2data/generation_labels_{tn}_{kv}_r{run+1}.npy", rescale=True, tn=tn, kv=kv, run=run) 
        
    

    #build_intensity_stats("./images", "./labels", "./", "generation_labels.npy", rescale=False)   # so this gives me all the means and all the standard deviations