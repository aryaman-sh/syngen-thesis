...
def dataloader(path, SPATIAL_SIZE, LABELS, ROI_CENTER_LOCATION, DATASETNAME):
    ...
    if DATASETNAME == 'BeLong_T2w':
        # Data prep
        train_files = [
            {'image': img, "label": seg} for img, seg in zip(images[:int(samples*.95)], segs[:int(samples*.95)])
        ]
        val_files = [
            {'image': img, "label": seg} for img, seg in zip(images[int(samples*.95):], segs[int(samples*.95):]) 
        ]
    ...
    return train_ds_loader, val_ds_loader, n_samples
...
def main():
    ...
    if args.datatype == 'organ':
        args.SPATIAL_SIZE = [288, 224, 96] #Should be multiples of 32
        args.LABELS = {
    ...
    # Models
    norm_model = torch.nn.DataParallel(Normalizer()).cuda()  # Wrap the normalizer model
    segmenter = torch.nn.DataParallel(DynUNet(
        spatial_dims=3,
    ...
    train_loader, val_loader, n_samples = dataloader(
        ...
        if epoch % 50 == 0 or epoch == 1:
            ...
            if args.model == 'NormSeg':
                ...
                if args.datasetName == 'BeLong_T2w':
                    ...
                    torch.save(segmenter.module.state_dict(), args.modelsDir +
                            str(n_samples) + 'samples_seed' + str(args.seed) + '_segmenter_model_only_' + str(epoch) + 'e_' + str(args.data) + '_' + str(args.datasetName) + '.pt')
        end = time.time()
        final_time = (end - init) / 60
        print('Loss: %s. Dice score in the validation set: %s. Time per epoch: %s minutes' %
