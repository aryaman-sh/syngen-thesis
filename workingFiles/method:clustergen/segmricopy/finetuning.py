...
def dataloader(path, SPATIAL_SIZE, LABELS, ROI_CENTER_LOCATION, DATASETNAME):
    ...
    if DATASETNAME == 'organ':
        ...
        train_files = [
            {'image': img, "label": seg} for img, seg in zip(images[:int(samples*.15)], segs[:int(samples*.15)])
        ]
        val_files = [
            {'image': img, "label": seg} for img, seg in zip(images[int(samples*.15):], segs[int(samples*.15):]) 
        ]
...
def main():
    ...
    segmenter.load_state_dict(torch.load(
            os.path.join('./synthOnly2/train_samples_446_samples_seed4_segmenter_model_only_50e_organ_organ'+ '.pt')))
    print("loaded model")
