...
def build_intensity_stats(list_image_dir,
    ...
    # save files
    np.save(os.path.join(result_dir, f'prior_means_{img_id}'), prior_means)
    np.save(os.path.join(result_dir, f'prior_stds_{img_id}'), prior_stds)

    return prior_means, prior_stds

def convert_string(input_str):
    parts = input_str.split('.')
    returnstr = parts[0]+'.'+parts[1]+".nii.gz"
    return returnstr

def main(data_dir):
    """
    Creates a new directory to store all step3 data
    """
    ...
    for file in files:
        ...
        with tempfile.TemporaryDirectory() as tempdir1, tempfile.TemporaryDirectory() as tempdir2:
            shutil.copy(f'{data_dir}/images/{convert_string(file)}', tempdir1) # image
            shutil.copy(f'./step1data/{file}', tempdir2)
            build_intensity_stats(f"{tempdir1}", f"{tempdir2}", "./step3data", f"./step2data/generation_labels_{img_id}.npy", rescale=True, img_id=img_id)
